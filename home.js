...
const addProject = (title, imageUrl) => {
  ...
  const src = /^https?:\/\//i.test(imageUrl)
    ? imageUrl
    : imageUrl.startsWith("/uploads/") || imageUrl.startsWith("/images/")
    ? imageUrl
...
const fetchProjects = async () => {
  ...
  const response = await fetch(`${window.API_BASE}/works`);
...
const applyCategoryFilter = async (id, event) => {
  ...
  const filteredProjects =
    id === 0
      ? allProjects
      : allProjects.filter((project) => project.categoryId == id);
...
const apiUrl = `${window.API_BASE}/works/`;
...
const deleteProject = async (id) => {
  ...
  const response = await fetch(`${window.API_BASE}/works/${id}`, {
    method: "DELETE",
    headers: {
      Accept: "application/json",
      "Content-Type": "application/json",
...
async function addNewProject(dataForm) {
  ...
  const url = `${window.API_BASE}/works`;
...
function setupAddProjectForm() {
  ...
  btnAddProject.addEventListener("click", async (e) => {
    ...
    if (res && !res.error) {
      ...
      const r = await fetch(`${window.API_BASE}/works`);
